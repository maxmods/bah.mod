			   ...
			   boolean shadow)
{
   ...
   */
   ...
   for (x = 0; x < (int) SIZEOF(bindings); ++x)
      bindCDKObject (vFSLIDER, widget, bindings[x].from, getcCDKBind, (void *)(long)bindings[x].to);
...
float activateCDKFSlider (CDKFSLIDER *widget, chtype *actions)
{
   ...
   if (actions == 0)
   {
	 ...
	 ret = injectCDKFSlider (widget, input);
   ...
   else
   {
      ...
      for (x=0; x < length; x++)
      {
	 ret = injectCDKFSlider (widget, actions[x]);
	 if (widget->exitType != vEARLY_EXIT)
	 {
	    return ret;
	 }
...
static int _injectCDKFSlider (CDKOBJS *object, chtype input)
{
   ...
   if (PreProcessFuncOf(widget) != 0)
   {
      ...
      ppReturn = PreProcessFuncOf(widget) (vFSLIDER, widget, PreProcessDataOf(widget), input);
   }
   ...
   if (ppReturn != 0)
   {
	 ...
	 switch (input)
	 {
      ...
      {
	 PostProcessFuncOf(widget) (vFSLIDER, widget, PostProcessDataOf(widget), input);
      }
   }
 ...
 */
static void _moveCDKFSlider (CDKOBJS *object, int xplace, int yplace, boolean relative, boolean refresh_flag)
{
   CDKFSLIDER *widget = (CDKFSLIDER *)object;
 ...
 */
void setCDKFSlider (CDKFSLIDER *widget, float low, float high, float value, boolean Box)
{
