			   ...
			   boolean shadow)
{
   ...
   */
   ...
   for (x = 0; x < (int) SIZEOF(bindings); ++x)
      bindCDKObject (vSCALE, widget, bindings[x].from, getcCDKBind, (void *)(long)bindings[x].to);
...
int activateCDKScale (CDKSCALE *widget, chtype *actions)
{
   ...
   if (actions == 0)
   {
	 ...
	 ret = injectCDKScale (widget, input);
   ...
   else
   {
      ...
      for (x=0; x < length; x++)
      {
	 ret = injectCDKScale (widget, actions[x]);
	 if (widget->exitType != vEARLY_EXIT)
	 {
	    return ret;
	 }
...
static int _injectCDKScale (CDKOBJS *object, chtype input)
{
   ...
   if (PreProcessFuncOf(widget) != 0)
   {
      ...
      ppReturn = PreProcessFuncOf(widget) (vSCALE, widget, PreProcessDataOf(widget), input);
   }
   ...
   if (ppReturn != 0)
   {
	 ...
	 switch (input)
	 {
      ...
      {
	 PostProcessFuncOf(widget) (vSCALE, widget, PostProcessDataOf(widget), input);
      }
   }
