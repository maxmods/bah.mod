...
tmx_map* tmx_load_buf(const char *buffer, int buflen, const char *filename) {
	tmx_map *map = NULL;
	const char *extension;
	int fchar;
	...
	/* is 'path' a JSON or a XML file ? */
	extension = strrchr(filename, '.'); /* First using the file extension */
	if (extension && (!strcmp(extension, ".tmx") || !strcmp(extension, ".xml"))) {
		map = parse_xml_buf(buffer, buflen, filename);
	} else if (extension && !strcmp(extension, ".json")) {
		map = parse_json_buf(buffer, buflen, filename);
	} else {
		/* open the file and check with the first character */
		if (buflen > 0) {
			fchar = buffer[0];
