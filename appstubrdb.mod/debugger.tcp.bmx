...
Global _bpMap:TMap = New TMap
Global _bpCount:Int
Global _bmdbBind:Int = 31666
Global _bmdbWait:Int = False
Global _bmdbSetup:Int = True

_initDebugger()

Function _initDebugger()
	' port to accept debugger connections against
	Local env:String = getenv_("BMDB_BIND")
	If env Then
...
Function ReadDebug$(skipNotAvail:Int = False)
	If _debugConnected Then
		...
		Local buf:Byte[1024], p:Int, n:Byte
		Local s:String
...
'WriteStdout "reading : " + n + "~n"
			
				buf[p]=n
				p:+1
				If p<>buf.length And p < count Then
					Continue
				End If
...
'If s Then ' just some debug
'WriteStdout "data = " + s + "~n"
'End If
		Return s
		'Return ReadStdin()
	End If
...
Extern
	Function _strlen:Int(b:Byte Ptr)="strlen"
End Extern
...
?MacOs
	...
	' but for remote debugging, assume we are debugging from elsewhere.
...
Function _checkConnection:Int()
	If Not _debugConnected Then
		...
		Global checking:Int
		If _debugSocket And Not checking And Not _bmdbSetup Then
...
'WriteStdout "debug session connected~n"
				End If
				checking = False
			'End If
		End If
			...
			If s Then
				...
				Else If s[..1].ToLower() = "b" Then
					...
					If off >= 1 Then
						...
						Local line:TDBLine = New TDBLine
						line.line = s[off + 1..s.length - 2].ToInt()
						
						Local list:TDBLine[] = TDBLine[](_bpMap.ValueForKey(file))
