...
Function DebugEscapeString$( s$ )
	s=s.Replace(Chr(0),"~~0")
	s=s.Replace(Chr(9),"~~t")
	s=s.Replace(Chr(10),"~~n")
	s=s.Replace(Chr(13),"~~r")
	s=s.Replace(Chr(34),"~~q")
	s=s.Replace(Chr(126),"~~")
	Return "~q"+s+"~q"
End Function
...
Function DebugDeclValue$( decl:Int Ptr,inst:Byte Ptr )
	...
	Default
		DebugError "Invalid decl typetag"
	End Select
...
Function DumpObject( inst:Byte Ptr,index )
	...
	If clas=Int Ptr Varptr bbStringClass
		...
		WriteDebug "~q"+String.FromShorts( Short Ptr(inst+12),(Int Ptr (inst+8))[0] )+"~q~n"
...
?
	...
	Repeat
		...
		Case "d"
			...
			Local inst:Int Ptr=Int Ptr Int( "$"+t )
