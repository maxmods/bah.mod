						       ...
						       struct libwebsocket *wsi)
{
	...
	context->protocols[0].callback(context, wsi,
		...
		wsi->user_space, (void *)(long)wsi->sock, POLLIN, context->protocols);
	...
	if (wsi->sock)
		context->protocols[0].callback(context, wsi,
		    LWS_CALLBACK_DEL_POLL_FD, wsi->user_space,
		    				    (void *)(long)wsi->sock, 0, context->protocols);
			 ...
			 struct libwebsocket *wsi, enum lws_close_status reason)
{
	...
	if (wsi->mode == LWS_CONNMODE_HTTP_SERVING_ACCEPTED && wsi->u.http.fd) {
		...
		wsi->u.http.fd = 0;
		context->protocols[0].callback(context, wsi,
			LWS_CALLBACK_CLOSED_HTTP, wsi->user_space, NULL, 0, context->protocols);
	}
...
just_kill_connection:
	...
	if (wsi->protocol && wsi->protocol->callback &&
		...
		wsi->protocol->callback(context, wsi, LWS_CALLBACK_CLOSED,
						      wsi->user_space, NULL, 0, wsi->protocol);
...
user_service:
	...
	if (pollfd) {
		...
		context->protocols[0].callback(context, wsi,
			...
			wsi->user_space, (void *)(long)wsi->sock, POLLOUT, context->protocols);
	}
#ifndef LWS_NO_EXTENSIONS
notify_action:
#endif
...
OpenSSL_verify_callback(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	...
	n = context->protocols[0].callback(NULL, NULL,
		LWS_CALLBACK_OPENSSL_PERFORM_CLIENT_CERT_VERIFICATION,
						   x509_ctx, ssl, preverify_ok, context->protocols);
		...
		struct libwebsocket_context *context,
			struct libwebsocket *wsi,
			 enum libwebsocket_callback_reasons reason, void *user,
							  void *in, size_t len, struct libwebsocket_protocols * protocol)
{
	int n;

	n = callback_function(context, wsi, reason, user, in, len, protocol);
	if (!n)
...
libwebsocket_create_context(struct lws_context_creation_info *info)
{
	...
	if (info->port == CONTEXT_PORT_NO_LISTEN) {
		...
		context->protocols[0].callback(context, NULL,
			...
			context->ssl_client_ctx, NULL, 0, context->protocols);
	}
#endif
	...
	for (context->count_protocols = 0;
		...
		info->protocols[context->count_protocols].callback(context,
			       NULL, LWS_CALLBACK_PROTOCOL_INIT, NULL, NULL, 0, &info->protocols[context->count_protocols]);
	}
