...
Rem
bbdoc: OSMesa Graphics/Max2D
End Rem
Module BaH.OSMesaMax2D
...
ModuleInfo "History: OSMesa port from original BRL.GLMax2D module"
...
Global _driver:TOSMesaGLMax2DDriver
...
Type TOSMesaGLImageFrame Extends TImageFrame
	...
	Function CreateFromPixmap:TOSMesaGLImageFrame( src:TPixmap,flags )
		'determine tex size
		...
		'done!
		Local frame:TOSMesaGLImageFrame=New TOSMesaGLImageFrame
		frame.name=name
...
Type TOSMesaGLMax2DDriver Extends TMax2DDriver

	Method Create:TOSMesaGLMax2DDriver()
		If Not OSMesaGLGraphicsDriver() Return Null
	...
	Method GraphicsModes:TGraphicsMode[]()
		Return OSMesaGLGraphicsDriver().GraphicsModes()
	End Method
	
	Method AttachGraphics:TMax2DGraphics( widget,flags )
		Local g:TOSMesaGLGraphics=OSMesaGLGraphicsDriver().AttachGraphics( widget,flags )
		If g Return TMax2DGraphics.Create( g,Self )
	End Method
	
	Method CreateGraphics:TMax2DGraphics( width,height,depth,pixdata,flags )
		Local g:TOSMesaGLGraphics=OSMesaGLGraphicsDriver().CreateGraphics( width,height,depth,pixdata,flags )
		If g Return TMax2DGraphics.Create( g,Self )
	End Method
	...
	Method SetGraphics( g:TGraphics )
		If Not g
			...
			OSMesaGLGraphicsDriver().SetGraphics Null
			Return
		EndIf
	
		Local t:TMax2DGraphics=TMax2DGraphics(g)
		Assert t And TOSMesaGLGraphics( t._graphics )

		OSMesaGLGraphicsDriver().SetGraphics t._graphics
	...
	Method Flip( sync )
		OSMesaGLGraphicsDriver().Flip sync
	End Method
	...
	Method CreateFrameFromPixmap:TOSMesaGLImageFrame( pixmap:TPixmap,flags )
		Local frame:TOSMesaGLImageFrame
		frame=TOSMesaGLImageFrame.CreateFromPixmap( pixmap,flags )
		Return frame
	End Method
...
rendering.
End Rem
Function OSMesaMax2DDriver:TOSMesaGLMax2DDriver()
	Global _done
	If Not _done
		_driver=New TOSMesaGLMax2DDriver.Create()
		_done=True
	EndIf
	Return _driver
End Function
