...
PluginReceiveResult NatPunchthrough::OnReceive(RakPeerInterface *peer, Packet *packet)
{
	...
	case ID_ALREADY_CONNECTED:
		RemoveFromConnectionRequestList(packet->systemAddress);
		break;
...
//	SystemAddress receiverAddrWithIPort;
	...
	while (i < connectionRequestList.Size())
	{
		...
		if (connectionRequestList[i]->advertisedPort!=0)
		{
			...
			{
				char out1[32];
				advertisedAddress.ToString(true, out1);
				LogOut(FormatString("Connecting to remotely advertised address %s\n", out1));
				advertisedAddress.ToString(false, out1);
				lastPortUsedToConnect=advertisedAddress.port;
				rakPeer->Connect(out1, advertisedAddress.port, connectionRequestList[i]->passwordData, connectionRequestList[i]->passwordDataLength);
				return RR_STOP_PROCESSING_AND_DEALLOCATE;
			}
		}
...
void NatPunchthrough::RemoveFromConnectionRequestList(SystemAddress systemAddress)
{
	unsigned i;
	i=0;
	DataStructures::List<SystemAddress> fallbackAddresses;
...
//				ConnectionRequest *connectionRequest = connectionRequestList[i];
//				rakPeer->CloseConnection(packet->systemAddress,false,0);
				connectionRequestList[i]->advertisedPort=packet->systemAddress.port;
//				rakPeer->Connect(packet->systemAddress.ToString(false), packet->systemAddress.port, connectionRequest->passwordData, connectionRequest->passwordDataLength);
			}
			break;
		}
	}
