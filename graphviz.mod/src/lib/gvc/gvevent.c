/* $Id: gvevent.c,v 1.52 2006/11/10 22:04:45 ellson Exp $ $Revision: 1.52 $ */
...
static void gvevent_find_current_obj(GVJ_t * job, pointf pointer)
{
    ...
    boxf b;
    double closeenough;
    pointf p, translation, scale, focus;
	...
	p.x = (pointer.x - (job->width / 2.0)) / scale.x + focus.x; /* BaH */
	//p.y = pointer.y / scale.y - translation.y;
	p.y = (pointer.y - (job->height / 2.0)) / scale.y + focus.y; /* BaH */
    }

#if 0
fprintf(stderr,"pointer = %g,%g scale = %g,%g translation = %g,%g focus = %g,%g graphpoint = %g,%g ; %g\n",
	pointer.x, pointer.y,
	scale.x, scale.y,
	translation.x, translation.y,
	focus.x, focus.y,
	p.x, p.y, (scale.x - 1.0) * job->width / 2);fflush(stderr);
#endif
...
static void gvevent_select_current_obj(GVJ_t * job)
{
    ...
    if (obj) {
        switch (agobjkind(obj)) {
        case AGGRAPH:
	    /*GD_gui_state((graph_t*)obj) |= GUI_STATE_VISITED; BaH */
	    GD_gui_state((graph_t*)obj) &= ~GUI_STATE_SELECTED;
	    break;
        case AGNODE:
	    /*ND_gui_state((node_t*)obj) |= GUI_STATE_VISITED; BaH */
	    ND_gui_state((node_t*)obj) &= ~GUI_STATE_SELECTED;
	    break;
        case AGEDGE:
	    /*ED_gui_state((edge_t*)obj) |= GUI_STATE_VISITED; BaH */
